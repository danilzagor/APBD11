...
namespace JWT.Controllers
{
    ...
    {
        [HttpPost("login")]
        public IActionResult Login(AuthController.LoginRequestModel model) 
        {
            ...
            return Ok(new AuthController.LoginResponseModel
            {
                Token = stringToken,
        ...
        [HttpPost("refresh")]
        public IActionResult RefreshToken(AuthController.RefreshTokenRequestModel requestModel)
        {
            var tokenHandler = new JwtSecurityTokenHandler();
            try
            {
        ...
        {
            var passwordHasher = new PasswordHasher<AuthController.User>();
            return Ok(passwordHasher.HashPassword(new AuthController.User(), password));
        }
        ...
        {
            var passwordHasher = new PasswordHasher<AuthController.User>();
            return Ok(passwordHasher.VerifyHashedPassword(new AuthController.User(), requestModel.Hash, requestModel.Password) == PasswordVerificationResult.Success);
        }
    }
